...
impl Cave {
    ...
    fn draw(&mut self, paths: Vec<Path>) {
        for path in paths.iter() {
            self.add_path(&path);
        }

        for x in 0..self.tiles.len() {
            self.tiles[self.lower_bound + 2][x] = Tile::Rock;
        }
    }
    ...
    fn produce_sand_until_block(&mut self) -> usize {
...
fn main() {
    ...
    let count = cave.produce_sand_until_block();
